...
import ProductScreen from '../landGllacrtraillscrnn/ProductScreen';
...
function IntroFlowScreen({navigation}) {
...
useEffect(() => {
  ...
  const Route = ({ isFatch }) => {
    if (!completeLink) {
      ...
      //return null;
      return <LandGllacrtraillLoaderScreen />;
    }
    ...
    return (
      <Stack.Navigator
        //initialRouteName="Loader" <Stack.Screen name="Loader" component={LandGllacrtraillLoaderScreen} />
  ...
  return (
    <NavigationContainer>
      {!isLoading ? <LandGllacrtraillLoaderScreen /> : <Route isFatch={route} />}  
    </NavigationContainer>
  );
}
